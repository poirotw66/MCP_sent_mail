...
app = Starlette(
    ...
    routes=[
        ...
        Route("/", endpoint=lambda request: Response(
            "MCP Email Sender Server is running!\n\nEndpoints:\n- GET /sse - SSE connection\n- POST /messages - MCP messages\n- GET /health - Health check",
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting MCP Email Sender Server on port {port}")
    print(f"📧 Email Account: {os.getenv('EMAIL_ACCOUNT', 'Not configured')}")
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=port,
